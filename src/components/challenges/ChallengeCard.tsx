...
export function ChallengeCard({ challenge, onComplete, onDelete, isToday }: ChallengeCardProps) {
  ...
  const currentTask = challenge.taskQueue?.[0]?.task || "Cargando siguiente misión...";
  ...
  const chartData = useMemo(() => {
    ...
    return data;
  }, [challenge.initialMetric, isQualitative]);
  ...
  return (
    <motion.div
      ...
      className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden group hover:shadow-2xl hover:shadow-emerald-500/5 hover:border-zinc-700 transition-all relative flex flex-col h-full"
    >
        ...
        <div className="flex-1 mb-8">
          {isQualitative ? (
            <div className="h-full flex flex-col justify-center">
              <p className="text-sm text-zinc-400 mb-2 font-medium">Misión de hoy</p>
              {completedToday ? (
                <div className="text-lg font-medium text-zinc-500 italic">Misión cumplida. Vuelve mañana para tu próximo 1%.</div>
              ) : (
                <p className="text-lg font-semibold text-zinc-100 leading-relaxed border-l-2 border-emerald-500 pl-4 py-1">
                  {currentTask}
                </p>
              )}
            </div>
        ...
        <div className="mt-auto">
          ...
          >
            {completedToday ? (
              ...
              isQualitative ? "Completar Misión" : "Completar 1% de hoy"
            )}
          </Button>
        </div>
      </div>

      {/* 30-day projection chart for quantitative only */}
      {!isQualitative && (
